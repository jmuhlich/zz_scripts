# Delete all but the first image from an .svs file and rename it to .tif.
# Cuts off the IFD list and truncates the file in place. Requires the
# typical SVS layout where the first IFD and all its data is contiguous
# and comes before any other IFD or other data. Some checking is performed
# to verify this and the script will exit if things look wrong.
...
svs_path = pathlib.Path(sys.argv[1])
if svs_path.suffix != ".svs":
    print("ERROR: Only .svs files are supported")
    sys.exit(1)

ts = TiffSurgeon(svs_path, writeable=True)
ts.read_ifds()
ifd0 = ts.ifds[0]
last_tile = np.argmax(ifd0.tags[324].value)
tile_stop = ifd0.tags[324].value[last_tile] + ifd0.tags[325].value[last_tile]
tags_stop = [tag.offset_range.stop for tag in ifd0.tags]
page_stop = max(tags_stop + [tile_stop])
if page_stop > ts.ifds[1].offset:
    print("ERROR: Detected interleaved data in SVS TIFF pages", file=sys.stderr)
    sys.exit(1)

ts.file.seek(ifd0.offset_range.stop - ts.offset_size)
ts.write(ts.offset_format, 0)
ts.file.seek(page_stop)
ts.file.truncate()
ts.file.close()

tif_path = svs_path.with_suffix(".tif")
svs_path.rename(tif_path)
w = ifd0.tags[256].value
h = ifd0.tags[257].value
num_strip = len(ts.ifds) - 1
print(f"{svs_path.name} -> {tif_path.name} : image is {w} x {h}; stripped {num_strip} image pages")
